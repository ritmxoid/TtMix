...
import { useLanguage, detectBrowserLanguage } from './context/LanguageContext';
...
export default function App() {
    ...
    if (saved) {
      const activePresetId =
        !saved.bgPresetId || saved.bgPresetId === 'midnight-violet'
          ...
          : saved.bgPresetId;
      const isOldDefaultFont = !saved.fontFamily || saved.fontFamily.includes('Montserrat');
      const fontFam = isOldDefaultFont ? "'Amatic SC', cursive" : saved.fontFamily;
      const fontSz = isOldDefaultFont && saved.fontSize === 72 ? 128 : (saved.fontSize || 128);

      const effectsMigrated = {
        glow: saved.effects?.glow ?? true,
        sparkle: isOldDefaultFont && saved.effects?.sparkle ? false : (saved.effects?.sparkle ?? false),
        fire: saved.effects?.fire ?? false,
        neon: saved.effects?.neon ?? false,
        shadow: saved.effects?.shadow ?? true,
        particles: isOldDefaultFont && !saved.effects?.particles ? true : (saved.effects?.particles ?? true),
      };
      ...
      return {
        ...
        fontFamily: fontFam,
        fontSize: fontSz,
        effects: effectsMigrated,
        proceduralMood: saved.proceduralMood || 'cosmic',
        proceduralSeed: saved.proceduralSeed || 1337,
        audio: {
          ...DEFAULT_STATE.audio,
          ...(saved.audio || {}),
          volume: saved.audio && saved.audio.volume > 0.12 ? saved.audio.volume : 0.7,
  ...
  // Safe no-op tour audio trigger (tour volume ducking is handled dynamically via audioMixer.setVolumeMultiplier)
  const handleSetTourAudio = useCallback(() => {}, []);
  ...
  const handleFileUpload = useCallback(async (file: File, isRestoration = false) => {
    ...
    if (isAudio) {
      ...
      const finishAudio = (dur: number) => {
        ...
        if (!isRestoration) {
          ...
          trackMediaUpload({ mediaType: 'audio', fileName: file.name, sizeBytes: file.size });
        }
      ...
      const loadVideoSource = (sourceUrl: string) => {
        ...
        const onReady = () => {
          ...
          if (!isRestoration) {
            ...
            trackMediaUpload({ mediaType: 'video', fileName: file.name, sizeBytes: file.size });
          }
        };
    ...
    if (isImage) {
      ...
      img.onload = () => {
        ...
        if (!isRestoration) {
          mediaManager.persistMedia('background', file, file.name, targetMime);
          trackMediaUpload({ mediaType: 'image', fileName: file.name, sizeBytes: file.size });
        }
  ...
  const handleExportMp4 = async () => {
    try {
      trackExportMp4Start({
        ...
        animationStyle: projectState.animationStyle,
      });
  ...
  return (
    <div className="min-h-screen bg-[#0F0F12] text-zinc-100 flex flex-col font-sans selection:bg-purple-600/30 selection:text-purple-200 overflow-x-clip w-full max-w-[100vw]">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-0 sm:p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-start">
        ...
        <div data-tour="workspace-tools-panel" className="order-2 lg:order-2 lg:col-span-7 space-y-4 sm:space-y-5 px-3 sm:px-0 pb-8">
          {/* Step 1: Text Input */}
          ...
          <div id="tool-section-3" data-tour="bg-tool-section" className="scroll-mt-16 transition-all duration-300 rounded-2xl">
            <BackgroundSection
              state={projectState}
              onChange={handleStateChange}
          ...
          <div className="bg-[#16161D]/85 border border-white/10 rounded-2xl p-4 sm:p-5 shadow-lg shadow-black/25 text-center space-y-2.5 mb-6">
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-purple-300">
                TtMix • {t('appSubtitle', 'Аниматор Текста')}
              </span>
            </div>
      ...
      <InteractiveTour
        ...
        onOpenFullscreen={() => setIsFullscreenOpen(true)}
        onCloseFullscreen={() => setIsFullscreenOpen(false)}
        forceOpenTour={isTourForceOpen}
        onCloseForceOpen={() => setIsTourForceOpen(false)}
        onSetTourAudio={handleSetTourAudio}
        isPinned={isPinned}
        onSetPinned={(pinned: boolean) => setIsPinned(pinned)}
        currentVolume={projectState.audio.volume}
        onTourActiveChange={setIsHelpTourActive}
      />

...
export const SavePresetModal: React.FC<SavePresetModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const defaultAuthorByLang =
        language === 'zh' ? '创作者' : language === 'ru' ? 'Автор' : 'Author';
      const savedAuthor = localStorage.getItem(LOCAL_STORAGE_LAST_AUTHOR) || defaultAuthorByLang;
      setAuthorName(savedAuthor);

      const defaultPresetPrefix =
        language === 'zh' ? '我的模板' : language === 'ru' ? 'Мой шаблон' : 'My Template';
      setPresetName(`${defaultPresetPrefix} #${Math.floor(Math.random() * 900 + 100)}`);
  ...
  const buildPresetObject = (): SavedPreset => {
    const finalName = presetName.trim() || (language === 'zh' ? '新模板' : language === 'ru' ? 'Новый шаблон' : 'New Template');
    const finalAuthor = authorName.trim() || (language === 'zh' ? '匿名' : language === 'ru' ? 'Аноним' : 'Anonymous');
  ...
  const getFormattedFileName = (preset: SavedPreset): string => {
    ...
    return `TtMix_preset__${safeName}__${safeAuthor}__${dateStr}.json`;
  };

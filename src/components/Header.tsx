    ...
    <header className="border-b border-white/10 bg-[#16161D]/95 backdrop-blur-md sticky top-0 z-40 px-2.5 sm:px-4 lg:px-8 py-2 sm:py-2.5 w-full overflow-x-clip">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-3 w-full min-w-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          {/* Group: Language & Help in one container */}
          <div data-tour="header-help-lang" className="flex items-center gap-1.5 shrink-0">
            {/* Language Selector */}
            ...
            {onOpenTour && (
              <button
                type="button"
                onClick={onOpenTour}
                className="text-xs text-purple-300 hover:text-white p-1.5 sm:px-2.5 sm:py-1.5 rounded-xl border border-purple-500/30 bg-purple-500/10 hover:bg-purple-500/20 transition-all flex items-center gap-1.5 cursor-pointer shrink-0 shadow-sm"
                title={t('howItWorks', 'Как это работает?')}
                data-tour="help-btn"
              >
                <HelpCircle className="w-3.5 h-3.5 text-purple-400" />
                <span className="hidden sm:inline font-medium">{t('howItWorks', 'Как это работает?')}</span>
              </button>
            )}
          </div>
          ...
          {fileName ? (
            <div data-tour="upload-btn" className="flex items-center gap-1 sm:gap-1.5 bg-purple-500/15 border border-purple-500/30 rounded-xl px-2 sm:px-2.5 py-1 sm:py-1.5 text-xs text-purple-200 shrink-0">

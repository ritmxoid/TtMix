...
export const VideoPreview: React.FC<VideoPreviewProps> = ({
  ...
  const handleExportPreset = (preset: SavedPreset, fileNameOverride?: string) => {
    ...
    const safeName = (preset.name || 'preset').replace(/[^\p{L}\p{N}_-]/gu, '_').replace(/_+/g, '_').slice(0, 40);
    const safeAuthor = (preset.author || 'author').replace(/[^\p{L}\p{N}_-]/gu, '_').replace(/_+/g, '_').slice(0, 30);
    const fileName =
      fileNameOverride || `TtMix_preset__${safeName}__${safeAuthor}__${dateStr}.json`;
    ...
    trackExportPreset({ presetName: preset.name });
  ...
  // When Fullscreen, Template Fullscreen, Recording Studio, or Export is active, halt preview canvas animation loop
  useEffect(() => {
  ...
  }, [isFullscreenActive, isExporting, bgMediaElement, isTourActive]);
  ...
  // Escape key to close popup or unpin back to compact 1/3 view
  useEffect(() => {
      ...
      <div ref={anchorRef} className="w-full relative">
        ...
        >
          ...
          >
            ...
            {/* Top-Right Clean Screen Eye Button */}
            <button
              type="button"
              ...
              className="absolute top-2.5 right-2.5 sm:top-3 sm:right-3 z-30 w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-black/55 hover:bg-black/80 active:scale-95 text-purple-300 hover:text-white backdrop-blur-md border border-white/20 hover:border-purple-400 shadow-xl transition-all cursor-pointer flex items-center justify-center pointer-events-auto group"
              title={hideControls ? t('showMenu', 'Показать меню') : t('cleanScreen', 'Чистый экран')}
              aria-label={hideControls ? t('showMenu', 'Показать меню') : t('cleanScreen', 'Чистый экран')}
            >
              {hideControls ? (
                <EyeOff className="w-5 h-5 text-purple-300 group-hover:scale-110 transition-transform" />
              ) : (
                <Eye className="w-5 h-5 text-purple-300 group-hover:scale-110 transition-transform" />
              )}
            </button>
